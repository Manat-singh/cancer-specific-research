...
class CancerTreatmentCluster:
    ...
    def prep_cluster():
        ...
        k_modes = KModes(n_clusters=2, init="Huang", n_init=1, verbose=1)
        ...
        combined_df.to_excel("cancer_exercise_diagnosis.xlsx", index=False)

        client = MongoClient(port=27017)
        db = client.adtProject
        db.cancer_exercise_diagnosis.drop()
        db.cancer_exercise_diagnosis.insert_many(combined_df.to_dict(orient='records'))

        plt.subplots(figsize=(15, 5))
        sns.countplot(x=combined_df['exercise'], order=combined_df['exercise'].value_counts().index,

...
class CancerTreatmentCluster:
    ...
    def prep_cluster():
        ...
        combined_df.to_excel("alcohol_diagnosis.xlsx", index=False)

        client = MongoClient(port=27017)
        db = client.adtProject
        db.cancer_alcohol_diagnosis.drop()
        db.cancer_alcohol_diagnosis.insert_many(combined_df.to_dict(orient='records'))

        plt.subplots(figsize=(15, 5))
        sns.countplot(x=combined_df['alcoholUse'], order=combined_df['alcoholUse'].value_counts().index,

...
class CancerTreatmentCluster:
    ...
    def prep_cluster():
        ...
        df = df[['diagnosis', 'district', 'tobaccoUse', 'alcoholUse', 'obese', 'exercise']]
        # print(len(df))
        df_copy = df.copy()
        ...
        combined_df.to_excel("cluster_patient_lifestyle.xlsx", index=False)

        client = MongoClient(port=27017)
        db = client.adtProject
        db.cluster_patient_lifestyle.drop()
        db.cluster_patient_lifestyle.insert_many(combined_df.to_dict(orient='records'))

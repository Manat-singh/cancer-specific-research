...
class CancerTreatmentCluster:
    ...
    def prep_cluster():
        ...
        k_modes = KModes(n_clusters=3, init="Huang", n_init=1, verbose=1)
        ...
        combined_df.to_excel("cancer_treatment_diagnosis.xlsx", index=False)

        client = MongoClient(port=27017)
        db = client.adtProject
        db.cancer_treatment_diagnosis.drop()
        db.cancer_treatment_diagnosis.insert_many(combined_df.to_dict(orient='records'))
        ...
        plt.show()
        # sns.catplot(data=combined_df, hue=combined_df['cluster_predicted'], x='diagnosis',
        #             y='treatment_type', size=2.5)

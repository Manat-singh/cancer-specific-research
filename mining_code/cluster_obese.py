...
class CancerTreatmentCluster:
    ...
    def prep_cluster():
        ...
        combined_df.to_excel("cancer_obese_diagnosis.xlsx", index=False)

        client = MongoClient(port=27017)
        db = client.adtProject
        db.cancer_obese_diagnosis.drop()
        db.cancer_obese_diagnosis.insert_many(combined_df.to_dict(orient='records'))

        plt.subplots(figsize=(15, 5))
        sns.countplot(x=combined_df['obese'], order=combined_df['obese'].value_counts().index,
